...
from players import Player, RandomPlayer, GreedyActionPlayer
...
class GameEvaluator:
    ...
    def evaluate(self, player: Player, n_games: int, budget: float, rounds: int) -> float:
        ...
        points = 0
        [w1, w2, ties] = self.play_games(int(n_games / 4), budget, rounds, player, p2)
        points += w1 + ties/2
        [w1, w2, ties] = self.play_games(int(n_games / 4), budget, rounds, p2, player)
        points += w2 + ties / 2
        [w1, w2, ties] = self.play_games(int(n_games / 4), budget, rounds, player, p3)
        points += w1 + ties / 2
        [w1, w2, ties] = self.play_games(int(n_games / 4), budget, rounds, p3, player)
        points += w2 + ties / 2
